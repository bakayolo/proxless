...
func ScaleUp(labelValue, namespace string) error {
	...
	if err != nil {
		...
		for _, item := range deploys.Items {
			go scaleUpDeployment(item, namespace, clientDeployment, ch)
		}
		wg.Wait()
	}
	return nil
...
func scaleUpDeployment(item v1.Deployment, namespace string, clientDeployment v1client.DeploymentInterface, ch chan error) {
